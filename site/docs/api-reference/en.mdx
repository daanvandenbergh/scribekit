...
The root entry re-exports `./shared`, `./blog`, and `./docs` in that order. Both the `Blog` and `Docs` class modules carry `import "server-only"` and read `node:fs`, so they resolve only in a server component, a route handler, or a build script. Importing either from a client component is a build error.
