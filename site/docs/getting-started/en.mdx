...
Scribekit also ships portable Claude Code skills, because the format is only half the problem: something has to write the pages. Three of them are in the published package today. `scribekit-blog` researches and writes posts, `scribekit-docs` writes documentation sourced from your own code, and `scribekit-hero` renders hero images deterministically from HTML and CSS. A fourth, `scribekit-docs-github-pages`, handles deployment and is not in the published 1.1.1 package yet. See [Skills](/docs/skills) for what each one does.
