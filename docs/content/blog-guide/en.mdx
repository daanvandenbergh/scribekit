...
- **The author keys are `author-image` and `author-url`**, kebab-case, in the YAML. The parsed fields on `PostMeta` are `authorImage` and `authorUrl`. Writing `authorImage:` or `authorUrl:` in the front-matter gets you nothing. `author-url` turns the author name (meta row and bio) into a link through your `linkComponent`; leave it out and the name stays plain text.
...
**One domain per locale.** If each language has its own domain (`https://example.nl/blog/x`, `https://example.de/blog/x`), pass `localeOrigins` instead of choosing a prefix: one origin per locale, e.g. an i18nkit `strategy: "domain"` instance's `i18n.domains`. No locale is then prefixed - every post lives at `/blog/<slug>` on its locale's domain - and every canonical, `og:url`, hreflang alternate, JSON-LD URL, sitemap entry and RSS link is built on the page locale's origin, with the hreflang alternates absolute so they cross domains. `siteUrl` defaults to the default locale's origin.

```ts
export const blog = new Blog({
    contentDir: "./blog",
    brandName: "Example",
    locales: [{ code: "nl" }, { code: "de" }, { code: "fr" }],
    defaultLocale: "nl",
    localeOrigins: { nl: "https://example.nl", de: "https://example.de", fr: "https://example.fr" },
});
```

Keep one `app/[lang]/blog/[slug]` tree and let your middleware rewrite each domain's bare `/blog/...` into it. The constructor throws on a missing or unknown locale, a non-`http(s)` origin, or `localeOrigins` together with `prefixDefaultLocale`. On localhost the other domains are unreachable, so leave `localeOrigins` unset in development (set `siteUrl` instead) to browse every language under `/<code>/blog/...`.
...
| `updatedLabel` | `BlogPage` | ``(d) => `Updated ${d}` `` |
