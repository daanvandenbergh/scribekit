/**
 * scribekit :: hero settings for the Scribekit documentation.
 * Brand + gradients for every docs hero image. Edit freely, then run the /scribekit-hero
 * `regenerate-docs-heroes` mode to re-render every hero. See skills/scribekit-hero/.
 *
 * Seeded from the blog surface's palette so both surfaces stay one visual family; only the
 * eyebrow differs (these are Scribekit's own docs, not the demo blog).
 */
...
/** Brand identity baked into every docs hero. Docs pages carry no byline: `DocMeta` has no author. */
export const brand = {
    ...
    accent: "109, 93, 246",
    eyebrow: "SCRIBEKIT DOCS",
};

/** The gradient options, built from the palette above. A page picks one by `name`. */
