...
| **Step 1** | Patch `next.config.*`: merge in `output: "export"`, `trailingSlash: true`, `images: { unoptimized: true }`, and a `basePath` / `assetPrefix` pair derived from `process.env.NEXT_PUBLIC_BASE_PATH`. |
