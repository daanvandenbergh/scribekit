...
| Signature | Emits |
| --- | --- |
| `buildPostMetadata(meta, site, translations = [meta.lang]): PageMetadata` | `` title: `${meta.title} \| ${site.brandName}` ``, `alternates.canonical`, OG `type: "article"`, `publishedTime: meta.date \|\| undefined`, `modifiedTime: meta.updated ?? (meta.date \|\| undefined)`, `images` with `alt`, `width: 1200`, `height: 630`, `locale`, `twitter.card: "summary_large_image"`, `twitter.site: site.twitterSite`. **Never sets `robots`** |
