...
| `rssFeedPath(site: SiteConfig, lang: string): string` | The conventional mount path for that locale's feed: the locale's index path plus `/rss.xml`, so `/blog/rss.xml` by default, or `/en/rss.xml` under `prefixDefaultLocale` |
