...
| `SiteConfig` | `siteUrl` and `brandName` are the **only two required** fields; plus `defaultAuthor?`, `basePath?`, `description?: LocalizedText`, `indexName?: LocalizedText`, `defaultLocale?`, `locales?`, `prefixDefaultLocale?`, `trailingSlash?`, `organizationId?`, `authorId?`, `websiteId?`, `twitterSite?`. `locales` is what lets the SEO builders read a locale's own `dateLocale` for `og:locale`; the backend classes set it from their own `locales` |
