...
`DocsConfig` extends `Partial<SiteConfig>`, so the site attributes are flat on the config object. The constructor throws only when `localeOrigins` is set - domain-per-locale URLs are supported by `Blog` only.
...
| `description` | `string \| Record<string, string>` | `undefined`, falls back to `` `The ${brandName} documentation.` `` | Index page description. A map is resolved per locale, falling back to `defaultLocale` |
| `indexName` | `string \| Record<string, string>` | `undefined`, falls back to `` `${brandName} Docs` `` | Index name: `CollectionPage` name and index OpenGraph title. Resolved per locale like `description` |
...
| `twitterSite` | `string` | `undefined`, which omits the tag | The site's X handle (`@example`), emitted as `twitter:site` on every page and index card |
...
| `docJsonLd(doc)` | `JsonLd`: `TechArticle` (with `mainEntityOfPage` and comma-joined `keywords`) + `BreadcrumbList` | `Error` when `site` is unset |
...
A locale's `dateLocale` is its full BCP 47 tag, and it drives `og:locale` as well as date formatting: a locale declaring `en-GB` publishes `og:locale` `en_GB`, where the bare `en` code would maximize to `en_US`.
...
| `localeOrigins` | `Record<string, string>` | `undefined` | One origin per locale (a domain-per-locale site). No locale is prefixed; every absolute URL uses the page locale's origin |
...
| `siteUrl` | `string` | `localeOrigins[defaultLocale]`, else `undefined` | Origin for `metadataBase` and absolute JSON-LD. Gates `site` |
| `brandName` | `string` | `undefined` | Title suffix, `og:siteName`, publisher. Gates `site` |
| `defaultAuthor` | `string` | `undefined` | Author fallback |
| `description` | `string \| Record<string, string>` | falls back to `` `The ${brandName} blog.` `` | Index description, also the RSS channel description. A map is resolved per locale, falling back to `defaultLocale` |
| `indexName` | `string \| Record<string, string>` | falls back to `` `${brandName} Blog` `` (bare `brandName` for the RSS `<title>`) | RSS channel title, `CollectionPage` name, index OpenGraph title |
...
| `twitterSite` | `string` | `undefined` | The site's X handle, emitted as `twitter:site` on every card |

`site` is built only when `brandName` and a `siteUrl` (given, or defaulted from `localeOrigins`) are both defined.

`localeOrigins` must name exactly the configured locale codes (the default locale alone when `locales` is unset), each an absolute `http(s)` URL; trailing slashes are stripped. The constructor throws `` Blog: `localeOrigins` needs an absolute http(s) origin for locale "fr"; got undefined. `` for a missing or malformed value, `` Blog: `localeOrigins` names "de", which is not a configured locale (en, fr). `` for an extra key, and a plain `Error` when combined with `prefixDefaultLocale: true`.
...
| `ogLocale(lang?)` | `string`, the `og:locale` tag (`en_GB`) for that language | no |
...
| `postJsonLd(post)` | `JsonLd`: `BlogPosting` (with `mainEntityOfPage` and comma-joined `keywords`) + `BreadcrumbList` | `Error` when `site` is unset |
...
| `authorUrl` | `string?` | no | `typeof === "string"`, strict. **The YAML key is `author-url`**, kebab-case. When set, `BlogPage` renders the author name as a link |
...
| Signature | Emits |
| --- | --- |
| `buildDocMetadata(meta, site, translations = [meta.lang]): PageMetadata` | `metadataBase`, `` title: { absolute: `${meta.title} \| ${site.brandName}` } `` (absolute, so a root `title.template` never appends the brand twice), `description`, `keywords`, `authors`, `alternates.canonical` and `languages`, OpenGraph (`type: "article"`, `publishedTime`, `modifiedTime: meta.updated ?? meta.date`, `images` with `alt`, `width: 1200`, `height: 630`, `locale`, `alternateLocale`), Twitter (`card: "summary_large_image"`, `site: site.twitterSite`, the same `images`). Sets `robots: { index: false }` **iff `meta.hidden`**. hreflang is `undefined` when `translations.length <= 1` |
| `buildIndexMetadata(site, lang?, langs = []): PageMetadata` | `` title: { absolute: `Docs \| ${site.brandName}` } ``, OG `type: "website"` with `` title: `${brandName} Docs` ``. No `robots` key; the index is never noindexed |
| `docJsonLd(meta, site, translations = [meta.lang]): JsonLd` | `@graph` of a `TechArticle` (`mainEntityOfPage: { "@type": "WebPage", "@id": <page url> }`; `keywords` as the comma-joined front-matter list, omitted when empty) and a `BreadcrumbList` (Home, Docs, the group name only, the page title). The `tab` never appears in JSON-LD breadcrumbs. When the page is translated, the `TechArticle` also carries `workTranslation` (an array, on the original-language version) or `translationOfWork` (on each translation) |
...
| Signature | Emits |
| --- | --- |
| `buildPostMetadata(meta, site, translations = [meta.lang]): PageMetadata` | `` title: { absolute: `${meta.title} \| ${site.brandName}` } `` (absolute, so a root `title.template` never appends the brand twice), `alternates.canonical`, OG `type: "article"`, `publishedTime: meta.date \|\| undefined`, `modifiedTime: meta.updated ?? (meta.date \|\| undefined)`, `images` with `alt`, `width: 1200`, `height: 630`, `locale`, `twitter.card: "summary_large_image"`, `twitter.site: site.twitterSite`. **Never sets `robots`** |
| `buildOverviewMetadata(site, lang?, langs = []): PageMetadata` | `` title: { absolute: `Blog \| ${site.brandName}` } ``, OG `type: "website"` with `` title: `${brandName} Blog` ``. hreflang only when `langs.length > 1` |
| `postJsonLd(meta, site, translations = [meta.lang]): JsonLd` | `@graph` of a `BlogPosting` (`mainEntityOfPage` and comma-joined `keywords`, as the docs builder does) and a `BreadcrumbList` of 3 items (Home, Blog, the title). A translated post also carries `workTranslation` / `translationOfWork`, as the docs builder does |
...
With `site.localeOrigins` set (a domain-per-locale site), every blog builder, `buildRssFeed` and `buildSitemap` build each URL on its page locale's origin with no locale prefix: `metadataBase` is that origin, and the canonical, `og:url` and hreflang `languages` become absolute URLs (root-relative ones cannot cross domains). Without it, the output is unchanged: root-relative metadata against `siteUrl`.
...
| `domainPerLocale` | `boolean?` | no | When `true`, no locale is prefixed - one domain per locale. Wins over `prefixDefaultLocale` |
...
| `domainPerLocale: true`, any locale | `/blog/post/` - the origin carries the locale |
...
| `LocalizedText` | `string \| Record<string, string>` - site-level copy that is either the same in every locale (a string) or given per locale code. A missing key falls back to `defaultLocale`, then to the builder's generic text, never to another locale's copy |
| `SiteConfig` | `siteUrl` and `brandName` are the **only two required** fields; plus `defaultAuthor?`, `basePath?`, `description?: LocalizedText`, `indexName?: LocalizedText`, `defaultLocale?`, `locales?`, `prefixDefaultLocale?`, `localeOrigins?: Record<string, string>`, `trailingSlash?`, `organizationId?`, `authorId?`, `websiteId?`, `twitterSite?`. `locales` is what lets the SEO builders read a locale's own `dateLocale` for `og:locale`; the backend classes set it from their own `locales` |
| `SitemapEntry` | `{ url: string; alternates?: { languages?: Record<string, string> } }` |
| `PageMetadata` | A structural subset of Next's `Metadata` with **no `next` type dependency**: `metadataBase?: URL`, `title?`, `description?`, `keywords?: string[]`, `robots?: { index?: boolean; follow?: boolean }`, `authors?: { name: string }[]`, `alternates?: { canonical?; languages? }`, `openGraph?` (its `images` entries take `url`, `alt?`, `width?`, `height?`), `twitter?` (same `images` shape, plus `site?`) |
...
`BlogPage` shows an `Updated <date>` item beside the publish date whenever the post's `updated` front-matter is later than its `date`.
...
| `description` | `string` | `site?.description`, resolved for `lang` when it is a per-locale map |
...
`blogLabels(lang)` returns `BlogLabels`: `empty`, `readMore`, `searchPlaceholder`, `loadMore`, `allCategories`, `back`, `onThisPage`, `similarPages`, `filterByCategory`, `writtenBy`, `publishedLabel(date)`, `updatedLabel(date)`, `readingLabel(minutes)`.
