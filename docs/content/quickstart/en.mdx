...
**Prerequisites:** a Next.js App Router app, and `@daanvandenbergh/scribekit` plus `next-mdx-remote` installed with the stylesheet imported in your root layout, as described in [Installation](/installation).
...
Later, when you rename a slug and list the old one in the instance's `redirects` map, spread `docs.getRedirectRefs()` into `generateStaticParams` as well. An unrendered slug 404s at the router before the component could redirect it, so the redirect would never fire. See the [Docs guide](/docs-guide).
