...
Prerequisites: the package installed, the stylesheet imported once, and a configured `Docs` instance. See [Installation](/installation) for that, [Build a docs site](/build-a-docs-site) for the end-to-end walkthrough, and [API reference](/api-reference) for every option, default, and token.
...
**There is no `:root` block in the stylesheet to override.** Every token exists only as an inline `var(--name, fallback)` at each call site, and that fallback is what you see when you declare nothing. Declaring the property anywhere it inherits from makes every call site pick it up. [API reference](/api-reference) lists the tokens.
